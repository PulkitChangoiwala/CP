...
//given a permutation find two equal length array a & b such that a merge b gives p

// logic: suppose max element is mx in p, so all elements in suffix starting with mx will belong as
//a contigous subarray in a either array a or b
...
v(int) subarray_len;
fr(i,1,ind.size()){
	subarray_len.pb(ind[i]-ind[i-1]);
}
// debug_arr(subarray_len);
if(subset_sum(subarray_len,n)) cout<<"YEs\n";
else cout<<"No\n"; 
