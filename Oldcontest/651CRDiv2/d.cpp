...
v(int) a;
int n,k;
bool check(int x, int curr){ //checking whether a subsequence with required conditionins is possible
	int ans=0;
	fr(i,1,n+1){
...
int binsearch(int l, int r){
	...
	while(l<r){
		...
		if(check(mid,0) || check(mid,1)){
			//check(mid,0) is checking if any subsequence with all a[i] at even position<=x
			//check(mid,1) is checking if any subsequence with all a[i] at odd position<=x
			r=mid;
		}
		else{
			l=mid+1;
...
//naive algo calculate all subseq O(n*2^n)
// using binary search check ans in 1 to 1e9 change upper bound and lower bound on the answer	
//x is a possible answer if we can generate subsequence that all s[i]<=x for all i either odd or even a
// and length of subsequence is greater than k
