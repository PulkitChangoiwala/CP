...
int o=0,e=0;
v(int) oi,ei;
fr(i,1,2*n+1){ cin>>a[i]; if(a[i]&1){ o++; oi.pb(i);} else {e++; ei.pb(i);}}

if(!(o&1) && !(e&1)){
	if(o>=2){oi.pop_back();oi.pop_back();o-=2;}
	else if(e>=2) {ei.pop_back();ei.pop_back(); e-=2;}
}
else { //both are odd, one odd other even case not possible
	oi.pop_back();ei.pop_back();o--; e--;
}
  
	fr(i,0,o-1) {
			cout<<oi[i]<<" "<<oi[i+1]<<endl;
			i++;
		}
		fr(i,0,e-1){
			cout<<ei[i]<<" "<<ei[i+1]<<endl;
			i++;
