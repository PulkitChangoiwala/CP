...
int compare(int i, int j,v(v(int))&same, string &s){
	if(i==0) return 0;
	int cmn = same[i-1][j-1];
	debug(mp(i,j));
	debug(cmn);
	if((j+cmn<=s.length() && s[i-1+cmn] <= s[j-1+cmn]))
		return cmn;
	else return -1 ;
}
...
void solve(){ 
	...
	for(int j=1; j<=n; ++j){
		for(int i=0; i<=j-1; ++i){
			...
			int tmp = compare(i,j,same,s); 
			if(tmp!=-1)
				dp[j] = max(dp[j], dp[i]+(n-j+1)-tmp);
		}
		ans = max(ans,dp[j]);
	}
	debug(dp);
