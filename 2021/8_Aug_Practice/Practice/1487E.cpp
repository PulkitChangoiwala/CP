...
void f(vector<int> &c, vector<int>&d, set<pair<int,int>> &st, 
	vector<set<int>> &adj
	){
	//updates c
	//st contains pair {minimum value, node number} of d
	int n3 = c.size(), n4 = d.size();
	set<pair<int,int>> st1;
	fr(i,0,n3){
		int node_c = i+1, tt = c[i];
		c[i] = -1;
		for(auto &el : st){
			int val = el.ff, node_d = el.ss;
			if(adj[node_c].find(node_d)== adj[node_c].end()){
				c[i] = tt+val;
				break;
			}
		}
		if(c[i]!=-1)
		st1.insert({c[i],node_c});
	}
...
void solve(){ 
	...
	int m1, m2, m3;
	v(set<int>) adj1(n1+1), adj2(n2+1), adj3(n3+1);
	cin >> m1; 
	fr(i,0,m1){
		int x, y;
		cin >> x >> y;
