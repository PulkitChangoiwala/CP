...
void solve(){ 
	...
	for(int el=n; el>=2; el-=2){
		v(int) ind(n+1);
		fr(i,0,n) ind[a[i]] = i+1;

		//setting el & el-1
		ans.pb(ind[el]);
		reverse(a.begin(), a.begin()+ind[el]); debug(a);
		fr(i,0,n) ind[a[i]] = i+1;
		ans.pb(ind[el-1]-1);
		reverse(a.begin(), a.begin()+ind[el-1]-1); debug(a);
		fr(i,0,n) ind[a[i]] = i+1;
		ans.pb(ind[el-1]+1);
		reverse(a.begin(), a.begin()+ind[el-1]+1); debug(a);
		fr(i,0,n) ind[a[i]] = i+1;
		ans.pb(3);
		reverse(a.begin(), a.begin()+3); debug(a);
		fr(i,0,n) ind[a[i]] = i+1;
		ans.pb(el);
		reverse(a.begin(), a.begin()+el); debug(a);
		fr(i,0,n) ind[a[i]] = i+1;
