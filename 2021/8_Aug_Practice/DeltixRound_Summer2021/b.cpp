...
int help(int st, vector<int>&a, set<int>&ind){
	int n = a.size();
	//st (1-st)
	int j = 0, ans = 0;
	debug(st);
	fr(i,0,n){
		if(ind.find(i)!=ind.end()) {ind.erase(i);}
		else {
...
void solve(){ 
	...
	fr(i,0,n) {
		cin >> a[i];
		if(a[i]&1) {ind.insert(i);}
		else {ind1.insert(i);}
 		a[i] = a[i]%2;
	}
	debug(a);
	int t1=-1, t2=-1;
	if(n%2==0 && ind.size()==ind1.size()){
		t1 = help(1,a,ind), t2 = help(0,a,ind1);	
	}
	else if(n&1){
		if(ind.size()==ind1.size()+1){
			t1 = help(1,a,ind);
		}
		else if(ind.size()==ind1.size()-1){
			t2 = help(0,a,ind1);
		}
	}
