...
void solve(){
	...
	fr(i,0,n){
		cin >> a[i];
		ind[a[i]].first += (i%2==0); //even pos
		ind[a[i]].second += (i%2!=0); //odd pos
	}

	sort(all(a));
	unordered_map<int,pair<int,int>> ind1;
	fr(i,0,n) {
		ind1[a[i]].first += (i%2==0); //even pos
		ind1[a[i]].second += (i%2!=0); //odd pos
	}

	for(auto &el : ind){
		if(el.second != ind1[el.first]){
			p1("NO");
			return;
		}
	}
