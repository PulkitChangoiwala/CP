...
void solve(){ 
	...
	for(int i=0; i<cnt.size(); ++i){
		auto pp = cnt[i];
		int ct = pp.ff; int ch = pp.ss;
		debug(ch);
		auto pp1 = cnt1[j];
		int ct1 = pp1.ff; int ch1 = pp1.ss;
		while(ct){
			// debug(make_pair(i,cnt[i].ff));
			...
			if(ct1){
				...
				cnt1[j].first--;
				mp1[ch+'a'].pb((ch1+'a'));
				debug(make_pair(ch,ch1));
			}
			if(!ct1)
			{
				j = (j+1)%(cnt.size());
				pp1 = cnt1[j];
				ct1 = pp1.ff, ch1 = pp1.ss;
			}
			// debug(make_pair(i,j));
		}
	}
