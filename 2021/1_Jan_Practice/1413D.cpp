...
stack<pair<int,int>> st; //it will store pair, indices and corresponding value
st.push({-1,n});
int pos = 0;
v(int) ans(n);
fr(i,0,2*n){
	...
	if(sign=='+'){
		...
		ans[ind] = x;
		st.top().s = max(st.top().s, x); 
		//because we have to check new number with x, whether it has smaller or larger value  
