...
void solve(){ 
	...
	if(a==b) {
		int ub = floor(1.0*(y)/(1.0*(a))); //leq
		int ub1 = floor(1.0*(x)/(1.0*(a))); //leq
		int ans = min(ub1,ub);
		p1(ans);
		return;
	}
	int m;
	int l = 0, r = ceil(1.0*(x+y)/(1.0*(a+b)));
	int ans = 0;
	swap(x,y);
	while(l<=r){
		m = (l+r)/2;

		int lb = ceil(1.0*(-y+m*a)/(1.0*(a-b))); //qeq
		int ub = floor(1.0*(x-m*b)/(1.0*(a-b))); //leq
		if( lb<=ub && lb<=m && ub>=0 ) {
			l = m+1;
			ans = max(ans,m);
		}
