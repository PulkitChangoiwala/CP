...
void solve(){ 
	...
	cin >> s;
	int n = s.length();
	int ans  = 0,cnt=0,previ=-1,qcnt=0;
	...
	for(int i=0; i<n; ++i){ //
		...
		if(s[i]=='0' && s[i-1]=='1' || s[i]=='1' && s[i-1]=='0' || s[i]=='?'){cnt++;}
		if(s[i]==s[i-1] && s[i-1]!='?' || s[i-1]=='?' && s[i]!='?') {
			...
			if(s[i-1]!='?') {cnt = 1;}
				...
				qcnt = i-previ-1;
				if(s[i]==s[previ] && (qcnt%2!=0) || s[i]!=s[previ] && (qcnt%2==0)) {ans -= cnt*(cnt+1)/2; cnt++;}
				else { ans-= (qcnt)*(qcnt+1)/2;  cnt = qcnt+1; } 
				//ans += (qcnt)*(qcnt+1)/2;
				//p3(i,cnt,ans);
			}
