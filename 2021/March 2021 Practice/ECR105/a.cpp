...
int x, y, z;
x = max(cnta,max(cntb, cntc));
z = min(cnta, min(cnta, cntc));
y = cnta+cntb+cntc - x-z;
char maxch;
if(x==cnta) maxch ='A'; else if(x==cntb) maxch = 'B'; else maxch = 'C';
...
if(x != y+z) p1("NO");
	...
	if(a[0]==maxch){
		...
		m[maxch] = 1;
		for(int it =0;it<3;++it){
			if(char('A'+it) != a[0] && char('A'+it) != maxch ){
				m[(char)('A'+it)] = 0;
			}
		}	
	}
