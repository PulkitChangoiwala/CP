...
if(l==n){cu++; cd++;}
if(r==n){cu++; cd++;}
if(cl>l || cr>r || cd > d || cu > u) {p1("NO"); return;}

if(u==n-1 && d==n-1) {
	if((cl+1 >l || cr+1 > r) && cl+2>l && cr+2>r) {p1("NO"); return;}
}
else if(u==n-1 || d == n-1){
	if(cl+1 >l && cr+1 > r ) {p1("NO");return;}

}

if(r==n-1 && l==n-1) {
	if((cu+1 >u || cd+1 > d) && cu+2>u && cd+2>d) {p1("NO"); return;}
}
else if(r==n-1 || l == n-1){
	if(cu+1 >u && cd+1 > d ) {p1("NO");return;}
