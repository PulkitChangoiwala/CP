...
/*
Algorithm:
Two balanced strings, a & b, of size n.

Total number of open brackets = n 

Each 1 introduces two open brakcets and each 0 introduces 1 open brackte => even number of 0 bits

Even no of 0 bits in 's' => even number of 1 bits in 's'


solutionn doesnot exists:
	1) s1 != 1 or sn != 1
	2) # 0 bits in s is not even

strategy to build a & b:

	- Let k be number of 1s in s
	- assign open brackers to first k/2 & close brackets to last k/2
	- For 0 bits in S, assign alternatively  
