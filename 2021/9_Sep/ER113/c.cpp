...
void solve(){ 
	...
	for(int i=0; i<n; ++i){ //position of max element
		int left = i, rem = left - cnt, right = n-1-i;
		int ways = (c.choose(n-cnt-1ll, rem));
		ways = (ways*(c.facs[left]))%mod;
		ways = (ways*(c.facs[right]))%mod;
