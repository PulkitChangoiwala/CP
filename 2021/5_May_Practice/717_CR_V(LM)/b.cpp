...
void solve(){ 
	...
	vector<vector<int>> xor_(n, vector<int>(n));
	fr(i,0,n){
		fr(j,i,n){
			if(i==j) xor_[i][j] = a[j];
 			else xor_[i][j] = xor_[i][j-1] ^ a[j];
		}
	}
	int pre,suf;
	fr(i,0,n-2){
		...
		for(int j=n-1; j>=i+2; --j) {
			if(xor_[j][n-1] == pre && xor_[i+1][j-1] == pre) {p1("YES"); return;}
		}
	}
	fr(i,0,n-1){
		if(xor_[0][i] == xor_[i+1][n-1]) {p1("YES"); return;}
	}	
	p1("NO");
