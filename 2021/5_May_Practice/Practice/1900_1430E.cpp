...
int countInversions(vector<int>&a, int l, int r){
	...
	int mid = (l+r)/2, cnt = 0, i =l, j = mid+1;
	vector<int> b;
	
	cnt = countInversions(a,l,mid) + countInversions(a,mid+1,r);
	
	while(i<=mid && j<=r){
		if(a[i]<=a[j]) {b.push_back(a[i]); i++;}
		else {cnt+= mid-i+1; b.push_back(a[j]); j++;}
	}
...
void solve(){ 
	...
	vector<int> p(n); //s.t. s[p[i]] = t[i]
	fr1(i,n-1,-1){
		p[n-i-1] = *(mp[s[i]-'a'].begin());
		mp[s[i]-'a'].erase(mp[s[i]-'a'].begin());
	}

	//debug_arr(p);

	p1(countInversions(p,0,n-1));
