...
void solve(){ 
	...
	let a < b 
	 ...
	 iff x mod a == x mod b mod a
	 ==> x  = x mod b +  q*a
	 Let x be of the form q1*b + k
	 Then q1*b + k = k + q*a

	 Thus we need q1 & q such that q1*b = q*a
	 =>
	 q1*b = q*a = t*lcm(a,b)

	 Thus, x = t*lcm(a,b) + k, where t is any integer and k is from 0 to b-1
	...
	*/
	...
	while(q--){
		...
		int len = r-l+1;
		int sub = 0;
		sub = b*(r/lc ) + min(r%lc + 1,b);
		sub -=  (b*(l/lc)+ min(l%lc +1, b) );
		if(l%lc <= b-1) sub++;
		len -= sub;
		cout<<len<<" ";	
	}
