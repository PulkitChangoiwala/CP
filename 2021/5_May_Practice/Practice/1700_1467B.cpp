  ...
  // debug_arr(st);
    ...
    fr(i,0,n){
        if(i==0){
            ...
            int tmp = 1ll*((st[i-1]!=-1) + (st[i]!=-1) + (st[i+1]!=-1));
            int tmp1 = tmp;

            //set a[i]  to a[i-1]
           //checking for a[i+1]
            if(i+2<n && (a[i-1]>a[i+1] && a[i+1] < a[i+2] || a[i-1]<a[i+1] && a[i+1] > a[i+2])) tmp--;
            
            
            
            //set a[i]  to a[i+1]
            //checking for a[i-1]
            if(i-2>=0 && (a[i-2]>a[i-1] && a[i-1] < a[i+1] || a[i-2]<a[i-1] && a[i-1] > a[i+1])) tmp1--;
           // p2(i,tmp1);
