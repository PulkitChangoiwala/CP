...
void solve(){ 
	...
	int ans = 0, dir = 1, curr = 0;
	int k, tmp1, tmp2, tmp;
	for(int i=1; i<=n;){
		tmp = t[i] + abs(x[i]-curr);
		if(tmp == t[i] ) {ans++; i++; continue;}
		k = upper_bound(t.begin()+i, t.end(), tmp-1) - t.begin();
		k--;
		dir = x[i] < curr? -1 : 1;
		for(int j = i; j<=k; ++j){
			tmp1 = dir*(t[j]-t[i]) + curr;
			if(j+1 == n+1 && i==n) tmp2 = dir*t[n+1];
			else if(j+1==k+1) tmp2 = x[i]; 
			else tmp2 = dir*(t[j+1]-t[i]) + curr;

			if(min(tmp1,tmp2) <= x[j] && x[j] <= max(tmp1,tmp2)) {
				ans++;
			}
		}
