...
void solve(){ 
	...
	vector<int> a(n);
	fr(i,0,n) cin >> a[i];
	if(a[n-1]>=2){p1(-1); return;}
	vector<pr> ans;

	int i = n-1,lastj=n-1;
	vector<int> cnt(n);
	vector<int> ones,twoscol, onescol;
	int k=0, k1=0, k2 = 0;
	fr1(j,n-1,-1){
		...
		if(a[j]==1){
			...
			ans.pb({i,j});
			if(k2<twoscol.size()) {ans.pb({i,twoscol[k2++]});}
			else if(k<onescol.size()) {ans.pb({i,onescol[k++]});}
			else { p1(-1); return;}
			lastj = j;
			twoscol.pb(j);
			cnt[i] += 2;
	...
	fr(i,0,n) if(cnt[i]>2){p1(-1); return;}
	p1(ans.size());
	for(auto el : ans){
		p2(el.f+1,el.s+1);
	}
