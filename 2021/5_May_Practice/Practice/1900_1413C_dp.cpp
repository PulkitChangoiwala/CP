...
void solve(){ 
	...
	sort(a.begin(), a.end());
	sort(b.begin(), b.end());
	vector<vector<pair<int,int>>> dp(n,vector<pr>(6));
	
	for(int j=0; j<6; ++j) { 
		dp[0][j] = {b[0]-a[j], b[0]-a[j]};
	}
	
	for(int i=1; i<n; ++i)
	{	for(int j=0;j<6;++j){
			tmp = b[i]-a[j];
			if(j==0) dp[i][j] = {min(dp[i-1][0].f,tmp), max(dp[i-1][0].s,tmp)};
			else {
				mn = dp[i-1][j-1].first;
				mx = dp[i-1][j-1].second;
				if(abs(min(mn,tmp)-max(mx,tmp)) < abs(dp[i][j-1].s - dp[i][j-1].f))
					dp[i][j] = {min(mn,tmp), max(mx,tmp)};
				else 
					dp[i][j] = dp[i][j-1];
			}
		}
	...
	int ans = 1e9;
	fr(j,0,6){
		ans = min(ans, abs(dp[n-1][j].f-dp[n-1][j].s));
	}
	p1(ans);
