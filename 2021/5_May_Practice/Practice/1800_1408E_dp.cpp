...
void solve(){ 
	...
	//debug_arr(x);
	for(int i=n; i>=1; i--){
		count[i] = lower_bound(x.begin(), x.end(), x[i-1]-k) - x.begin();
		count[i] = i - count[i];
	}
	//debug_arr(count);
	for(int i=1;i<=n;++i){
		//for 1 platforms
		dp[i][0] = max(dp[i-1][0],count[i]);
		//for 2 platforms
		j = i-count[i];
		dp[i][1] = max(dp[i-1][1], dp[j][0]+i-j);
	}

	p1(max(dp[n][0], dp[n][1]));
