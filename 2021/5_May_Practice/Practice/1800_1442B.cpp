...
void solve(){ 
    ...
    vector<int> a(n), b(k);
    vector<set<int>> neighbours(n+1);
    //int x,y;
    vector<bool> isPresent(n+1, false), block(n+1, false);
    fr(i,0,n) { cin >> a[i];}
    fr(i,0,k) { cin >> b[i]; isPresent[b[i]] = true;}
    ...
    int ind = 0, cnt = 0, n1,n2, ans =  1;
    fr(i,0,k){
        int sz = neighbours[b[i]].size();
        if(sz==0) {
            p1(0);
            return;
        }
        else if(sz==1) {
            n1 = *(neighbours[b[i]].begin());
            if(isPresent[n1]) {p1(0); return;}

        }
        else if(sz==2) {
            n1 = *(neighbours[b[i]].begin());
            neighbours[b[i]].erase(n1);
            n2 = *(neighbours[b[i]].begin());
            neighbours[b[i]].insert(n1);
            if(isPresent[n1]&&isPresent[n2]) {p1(0); return; }
            if((!isPresent[n1]) && (!isPresent[n2])) {ans = mul(2,ans);}
            if(isPresent[n1]) n1 = n2;
        }
        
        neighbours[b[i]].erase(n1);
        neighbours[n1].erase(b[i]);
        if(neighbours[n1].size()>0){
            int tmp = *(neighbours[n1].begin());
            neighbours[tmp].erase(n1);
            neighbours[tmp].insert(b[i]);
            neighbours[b[i]].insert(tmp);
        }
        isPresent[b[i]]=false;
    }
    p1(ans);
