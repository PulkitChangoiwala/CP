...
From n-1 to i we find minimum height(>=k) possible,
then check if ( suffix[i] - minimum_height(>=k) ) >=k 
then n-i is one of the possible answer

We find minimum n-i of all possible answers
...
v(v(int)) dp(n+1, v(int)(k+1, INT_MAX));
...
for(int i=n-1; i>=0; --i){
	for(int j=k; j>=0; --j){
		...
		//dp[i][j]= minimum height >= j, possible by using i to n-1 elements
		if(h[i]>=j){
			dp[i][j]=h[i]; 
		}

		else{
			if(dp[i+1][j - h[i]]==INT_MAX){
				dp[i][j] = INT_MAX;
			}
			else 
	...
	if(dp[i][k]!=INT_MAX && suffix[i]-dp[i][k] >= k) {
		p1(n-i);
		f=1;
		break;
	}
