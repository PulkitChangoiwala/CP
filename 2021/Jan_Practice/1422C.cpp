...
void solve(){ 
string s; cin>>s;
int n = s.size(); //n is the length of the number
...
for(int i=2;i<=n;++i){
	int temp = (i-1)*(i)/2;
	dp[i] = mult(temp, (s[i-1]-'0'));
	dp[i] = add(dp[i],mult(dp[i-1],10)); 
	number = add(mult(number,10),s[i-1]-'0');
	pref[i] = add(pref[i-1], number);
	dp[i] = add(dp[i],pref[i-1]);
