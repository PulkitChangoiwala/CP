...
//calculating max possible operations

int ans = 0, one = 0; // note ans <= a.size();
int len = a.size();
fr(i,0, len){
	if(a[i]>1) {
		if(a[i]>one+1) { ans = ans + one + 1; one = 0;}
		else if(a[i]>one) {ans = ans + one; one = 1; }
		else { ans = ans + a[i]-1; one = one - a[i] + 1 + 1;}
	}
	else {
		one++;
	}
