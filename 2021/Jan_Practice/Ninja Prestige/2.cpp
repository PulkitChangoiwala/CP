...
int n,k;
int cnt;
bool possible(double avg, int n){
	
	cnt++;
	v(double) suff(n);
	suff[0] = a[0]-avg;
	for(int i=1;i<n;++i){
		suff[i] = suff[i-1] + a[i] - avg;
	}

	//Check whether any subsequence with sum greater than avg is present or not
	//subsequence should be of atlease length k

	double mn = 0.0;
	for(int i = k-1; i<n;++i){

		if(suff[i]>=mn) return true;
		mn  = min(mn, suff[i-k+1]);
	}
return false;
