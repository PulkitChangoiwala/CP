...
v(v(v(int))) dp;
int n;
int check(int city, int p, int f){
	int ans = 0;
	if(f==0 && city ==0) return 0;
	if(f==1 && city == n) return 0;
	if(dp[city][p][f]!=-1) return dp[city][p][f];
	if(f){
		if(s[p][city+1] == 'R'){
			ans = 1+check(city+1,1-p,f);
		}
	}
	else{
		if(s[p][city] == 'L'){
...
cin>>n; //n+1 cities
dp.clear();
dp.resize(n+1, v(v(int))(2, v(int)(2,-1)));
s.clear();
cin>>s[0];
s[0] = "_"+s[0];
