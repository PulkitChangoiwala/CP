...
int fun(vector<int> &a, vector<int> &b){
	...
	for(int i=m-1; i>=0; --i){
 		...
 		if(it<n && a[it]==b[i]) pref[i] = pref[i+1]+1;
	...
	fr(i,0,m){
		//let some a[j] is pushed to b[i]
		int it = upper_bound(a.begin(), a.end(), b[i])- a.begin(); //note it = j+1
		//There will be block of a[0] to a[j] (total of j+1 elements) ending at b[i] thus
		int it2 = lower_bound(b.begin(), b.end(), b[i]-it+1) - b.begin();

		int temp = (i-it2+1) + pref[i+1];
		ans = max(ans, temp);
...
for(int i=0; i<m; ++i){
	if(b[i]<0) { negb.pb(-1*b[i]);  }
	else {posb.pb(b[i]);}
}
reverse(negb.begin(), negb.end());

p1(fun(posa,posb)+fun(nega,negb));
