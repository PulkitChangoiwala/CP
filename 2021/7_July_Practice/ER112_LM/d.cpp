...
periodic => no pal
- periodic string does not contain any
palindrome
...
so convert substring to periodic 
substring
...
void solve(){ 
	...
	string s;
	cin >> s;
	v(v(int)) cnt(n+1, v(int)(3));

	fr(i,1,n+1){
		fr(j,0,3){
			cnt[i][j] = (i<3? 0: cnt[i-3][j]) + (s[i-1]-'a'==j);			
		}
	}
	...
	fr(i,0,m){
		...
		if(l==r) {p1(0);}
			...
			int ans = 1e9;
			do{
				int tans = 0, total, it1, it2 = r+1;
				for(auto ch : ord){
					total = (it2-l+2)/3;
					it1 = it2 - (total)*3;
					it1 = max(0, it1);
					tans +=  total - (cnt[it2][ch] - cnt[it1][ch]);
					it2--;
				}	
				ans = min(ans, tans);
