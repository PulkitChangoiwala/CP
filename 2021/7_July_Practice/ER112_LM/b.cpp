...
void solve(){ 
	ll w,h,x1,x2,y1,y2,w1,h1,w2,h2;
	...
	if(h2 > h-h1 && w2 > w-w1){p1(-1);}
	else if(w2 <= x1 || w2 <= w-x2) {p1(0);}
	else if(h2 <= y1 || h2 <= h-y2) {p1(0);}
	else {
		ll dx=0,dy=0;
		// dx = w2 - max(x1, w-x2);
		// dy = h2 - max(y1, h-y2);
		dx = w2 - x1; 
		if(dx>w-x2) dx = 1e18;
		if(w2-(w-x2)<=x1) dx = min(dx, w2-(w-x2));
		dy = h2 - y1; 
		if(dy>h-y2) dy = 1e18;
		if(h2-(h-y2)<=y1) dy = min(dy, h2-(h-y2));

		long double tt = 1.0;
		// long double ans = sqrt(tt*dx*dx + tt*dy*dy);
		long double ans = min(dx,dy);
		// ans *= 1.0;
		// p1(ans);
