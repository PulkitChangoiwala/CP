...
for(int i=0;i<n;++i) cin>>h[i];
	...
	//Let for i-1 th segment [li-1, ri-1] height possible 
	//Then for ith segment
	// li = max(li- (k-1), hi)
	// ri = min(hi+ k-1 , ri + k- 1)

	pair<int, int> range;
	range.first = h[0]; range.second = h[0];
	bool flag = false;
	for(int i=1;i<n;++i){
		int l = range.f; int r = range.s;
		range.f =  max(l-(k-1), h[i]);
		range.s =  min(r + k-1, h[i]+ k-1);
	...
	if(flag || h[n-1] <range.f || h[n-1] > range.s){
		p1("No");
	}
