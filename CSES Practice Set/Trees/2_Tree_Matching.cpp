...
int ans(int i, int p){
	...
	if(adj[i].size()==0) {dp[i][0]=0; dp[i][1]=0; return 0;}
	...
	for(auto el : adj[i]){
		if(el==p) continue;
		ans(el,i);
		dp[i][0] += max(dp[el][0],dp[el][1]); //i is not included
		dp[i][1] += max(dp[el][0],dp[el][1]); //i is included
		temp = min(temp,dp[el][1]-dp[el][0]); //Finding child node to which an edge from i is to be included
	}

	if(temp<0) temp = 0; //If there exists a child which is not included in its subtrees maximum matching
	dp[i][1] += 1 - temp; 
