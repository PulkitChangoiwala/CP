...
int depth(int s, int p){
	int res = 0, res_ = 0, temp;
	if(adj[s].size()==1 && adj[s][0] == p) return 0;
	for(auto v : adj[s]){
		if(v!=p){
			temp = depth(v,s)+1;
			if(temp > res) { res_ = res; res = temp; }
			else if(temp<=res && temp>res_) res_ = temp;
		}
	}
	diameter = max(diameter, res+res_);
//	p3(s, res, res_);
	return res;
}
