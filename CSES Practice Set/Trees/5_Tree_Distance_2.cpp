...
vector<int> cnt,child_sum,sum;
int a,b,curr;


void dfs(int s, int p, int d){
    if(adj[s].size()==1 && adj[s][0] == p) {cnt[s]=0;child_sum[s] = 0; return;}
    cnt[s] = 0;
    child_sum[s] = 0;
    for(auto v : adj[s]){
        if(v!=p){
