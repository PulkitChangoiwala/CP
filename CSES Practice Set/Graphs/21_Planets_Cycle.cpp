...
void solve(){ 
	...
	fr(i,1,n+1){
		if(!vis[i]){
			length = 0;
			repeat = 0; //first visited encounterd
			base = 0; //number transportation for the node "repeat"
			flag = 0;
			dfs(i);
			while(!q.empty()){
				v = q.front();
				...
				//Case1: First Repeat, Start Node Of A Cycle
				if(v==repeat){
					ans[v] = q.size()+1;
					flag = 1;
					continue;
