...
void djikstra(int start){
	...
	mx.resize(n+1, INT_MIN);
	cnt_route.resize(n+1,0);
	cnt_route[start] = 1;
	mx[start] = 1;
	mn[start] = 1;
	set<pair<int,int>> st;
	...
	while(!st.empty()){
		...
		for(auto itr : adj[v]){
			...
			if(it != st.end())
			{
				if(dis[u] >  dis[v] + wt && dis[v] != inf){
					dis[u] = dis[v] + wt;
					st.erase(it);
					st.insert({dis[u],u});
					cnt_route[u] = cnt_route[v];
					mn[u] = mn[v]+1;
					mx[u] = mx[v]+1;
				}
				else if(dis[u] == dis[v]+wt && v != inf){
					cnt_route[u] = (cnt_route[u] + cnt_route[v])%mod;
...
void solve(){ 
	...
	djikstra(1);
	p4(dis[n], cnt_route[n], mn[n]-1, mx[n]-1);
