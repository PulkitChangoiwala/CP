...
void dijkstra(vector<vector<pair<int,int>>> &adj, vector<int> & dis, int s){
	...
	while(!st.empty()){
		...
		for(auto edge : adj[v]){
			...
			if(vis[u]) continue;
			auto itr = st.find({dis[u],u});
			if(itr != st.end()){
				if((*itr).first > dis[v]+wt){
					dis[u] = dis[v]+wt;
					st.erase(itr);
					st.insert({dis[u],u});
				}
...
void solve(){ 
	...
	int a,b,c;
	vector<vector<pr>> adj(n+1), adj_r(n+1);
	vector<int> dis_1, dis_n;
	...
	fr(i,1,n+1){
		for(auto el : adj[i]){
			adj_r[el.first].pb({i,el.second});
		}
	}


	//distance from 1 to i
	dijkstra(adj, dis_1, 1);

	//distance from j to n
	dijkstra(adj_r,dis_n,n);
	...
	fr(i,1,n+1){
		for(auto el : adj[i]){
			ans = min(ans, dis_1[i]+el.second/2 + dis_n[el.first]);
		}
	}
