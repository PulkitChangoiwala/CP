...
int find(int p, int Q){
	...
	for(int i=2; i*i<=q;++i){
		if(q%i) continue;
		//we need to reduce some power of "i"  from p such that resulting number is no longer divisible by q
		int temp = p;
		//here we do not consider much about composite factors as it can be proved that optimal answer is only when we 
		//remove some power of prime factor from p
		while(temp%i ==0 && temp%q == 0){ temp /= i; }
		if(temp%q != 0) {ans = max(ans, temp);}
		int j=q/i;
		temp =p;
		while(temp%j ==0 && temp%q == 0){ temp /= j; }
		if(temp%q != 0) {ans = max(ans, temp);}		
	}
	if(ans == -1){ //when q is prime number
		while(p%q==0) p/=q;
		return p;
	}
