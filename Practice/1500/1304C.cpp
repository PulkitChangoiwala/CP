...
void solve(){ 
//for each t, find temp range possible 
// then check that temp range intersects with the
// customer's favoured temp range
int n,m;cin>>n>>m;
int mn=m,mx=m;
int tp=0;
fr(i,0,n){
	...
	mx+=t-tp; mn-=t-tp;
	tp=t;
	if(l<=mx && h>=mn){mx=min(h,mx); mn= max(l,mn);}
	else {cout<<"NO\n"; 
	fr(j,i+1,n){
	cin>>t>>l>>h;
	} //Removing elements of this testcase
	return;}		
