...
v(v(int )) adj;
//reursion used is
// expected path tree T = {1/(# subtrees)}*{sum of all expected path length in subtrees}
double dfs(int v =1, int p =0){ //return expected path

	double sum=0;
	for(auto u : adj[v]){
		if(u!=p){
			sum+= dfs(u,v)+1; //1 for current edge

		}
	}
	return sum?(sum/(adj[v].size() - (p!=0))) : 0 ; 
	// to avoid 0/0 condition when tree consists of one and only one node
}
void solve(){ 
...
while(!q.empty()){
	...
	for(auto u : adj[v]){
		if(!visited[u]){
			prob[u] = prob[v]*(child[v]);
			visited[u]=true;
			q.push(u);
		}
	}
	if(child[v]==0){
		ans = (double)(ans + (double)((level[v]+0.00000000)/prob[v]));
	}
