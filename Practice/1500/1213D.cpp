...
x can be from 0 to max(a[i])

store for each x... at div[i] number of division required to convert a[i] to x
...
for(auto x : store){ //O(mxa*n*logn)
	if(x.size()>=k){
		sort(x.begin(), x.end());
		ans=min(ans,accumulate(x.begin(),x.begin()+k,0ll));
	}
}
cout<<ans<<endl;
  return;} // solve ends 
