...
/*
logic: two cards determine the third card.
for all pairs we search number of possibility of third card

n^2 pairs.
we can search third card in O(k*n)
overall O(k.n^3)

so we need to optimieze the search for third card
using map we can find the number in O(k*logn)
overall O(k.logn.n^2)
...
*/
int n,k; 
string third(string &s, string &t){
	string res = "";
	fr(i,0,k){
		if(s[i]==t[i]){
			res+=s[i];
...
fr(i,0,n){
	fr(j,i+1,n){
		string temp = third(s[i],s[j]);
		auto itr = store.find(temp);
		if(itr!=store.end() && s[i]!=temp && s[j]!=temp)
		ans+= (itr)->second;
		else if(itr!=store.end() && s[i]!=temp && s[j]!=temp)
		ans+= ((itr)->second)-1;	
	}
}
 cout<<ans/3<<endl;
