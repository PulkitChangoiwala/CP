...
sort(a.begin(),a.end());
//debug_arr(a);
int cnt = (n-1)/2; // max possible
int j = cnt, i;
int ans =0;
v(int) res;
for(i=0;i<cnt;){
	if(j+1<n && a[i]<a[j] && a[i] < a[j+1]){
		ans++;
		res.pb(a[j]); res.pb(a[i]); //res.pb(a[j+1]);
		j++;
		i++;
	}	
	else{
...
for(;i<cnt;++i){ res.pb(a[i]);}
for(;j<n;++j){res.pb(a[j]);}


//if(n== 99856) p1(res.size());
p1(ans);
debug_arr(res);
