...
int n; cin>>n;
v(int) cnt(n+1);
fr(i,0,n) {int x; cin>>x; cnt[x]++;}

/*
for x be the minimal element in an tuple different possibilties are:
...
so we need only count of these numbers then we can use PnC to find possible tuples
*/
//debug_arr(cnt);
int ans =0;
if(n<3) {p1(ans); return;}
for(int x=1;x<=n;++x){
	if(!cnt[x]) continue;
	if(cnt[x]>=3) ans += (cnt[x]*(cnt[x]-1)*(cnt[x]-2)/6);
	if(x+1 <= n){
		if(cnt[x]>=2) ans += (cnt[x]*(cnt[x]-1)*(cnt[x+1])/2);
		if(cnt[x+1]>=2) ans += (cnt[x]*(cnt[x+1])*(cnt[x+1]-1)/2);
	}

	if(x+2 <= n){
		ans += (cnt[x])*(cnt[x+1])*(cnt[x+2]);
		if(cnt[x]>=2) ans += (cnt[x]*(cnt[x]-1)*(cnt[x+2])/2);
		if(cnt[x+2]>=2) ans += (cnt[x]*(cnt[x+2])*(cnt[x+2]-1)/2);
	}
}
p1(ans);
