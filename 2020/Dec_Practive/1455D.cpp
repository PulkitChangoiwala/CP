...
Run i from  0 to n
At i th iteration
1) Check if array from ai to a[n] is sorted then return count
2) else  if(x <= a[i]) i++;
3) else swap(x, a[i]) count++, i++;
...
for(int i=0;i<n;++i){
	...
	if(sorted(a,0)) {flag=1; break;}
	if(a[i]>x) {
		int temp = x; x=a[i]; a[i]=temp; //swapping
		cnt++;
	}
}
...
// Method 2
if(n==1) return 0;
if(sorted(a,0)) return 0;

for(int i=0;i<n;++i){

	if(x<a[i]) {
		if(i<n && sorted(a,i+1)){
			if(a[i]<=a[i+1]) return cnt;
			else if(x<=a[i+1]) return cnt+1;
			else return -1;
		}
		else if(i<n) {
			int temp = x; x=a[i]; a[i]=temp; //swapping
