...
fr(i,0,n){
	...
	int add=k-a[i]%k;
	/*d[k-a[i]%k]++;
	if(a[i]%k) t=max(t,d[k-a[i]%k]);*/
	auto itr=store.find(add);
	if(itr!=store.end()){
		int tmp = itr->second;
		itr->second= tmp+1;
		if(a[i]%k) t=max(t,tmp+1);

	}
	else {
	store.insert({add,1});
	if(a[i]%k) t=max(t,1ll);

}
	
}
if(store.find(k)->second==n) cout<<0<<endl;
else {
    int ans=k*(t-1);
    // cout<<ans<<endl;
    int last=-1;
	for(auto it : store){
		if(it.second==t && it.first!=k){
