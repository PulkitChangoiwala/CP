...
//n*a +b -c = m
//m+c-b should be divisible by a

for(int a = l; a<=r;++a){
	int rem = m%a;
	int tmp; tmp=rem*-1;
	if(tmp<l-r || m+tmp==0) tmp = a-rem;
	rem = tmp;
	if(rem == 0) {p3(a,a,a); return;}
	else if(l-r<=rem<=r-l) {
		int b,c;
		if(rem<0) {c = l; b=l+abs(rem); if(b>r) continue;}
		else {b=l; c=l+rem; if(c>r) continue;}
		p3(a,b,c); return;}
