...
//if A is answer than for all C[i] | A = A
// A is<= 2^9 -1 (=511), so we can check for all possible
//answers and return minimum

fr(ans,0,512){
	int cnt=0;
	fr(i,0,n){
		fr(j,0,m){
			if(((a[i]&b[j])|ans) == ans){cnt++; break;} 
		}
	}
	if(cnt==n) {p1(ans); break;}
}
