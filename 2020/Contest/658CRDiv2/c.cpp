...
//O(n) with 3*n operation
for(int i=0;i<n;++i){
	if(s[i]!=t[i]) {
		//we need to change i the bit
		ans.pb(i+1); //bring bit to first
		ans.pb(1); // change bit
		ans.pb(i+1); // restore other bits back
	}
}
 cout<<ans.size()<<" ";
 debug_arr(ans);
