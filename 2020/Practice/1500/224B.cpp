...
/*
logic:
 de stores number of diff element.

 if(de<k) not possible 
 else{
	
	initially assume [l,r] = [1,n] as answer
	cnt[a[r]]--, if(cnt[a[r]]==0) de--; 
	if(de>=k) decrese r by 1,
	else cnt[a[r]]++;

	same procedure for l, but increase l by 1 instead of decreasing

 }
*/ 

if(de<k) {cout<<-1<<" "<<-1<<endl;}
else{
	int l,r;
	for(r=n; r>0; ){
		cnt[a[r]]--;
