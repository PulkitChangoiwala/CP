...
int n; cin>>n; 
v(int) a(n);
unordered_map<int,int> lastindx, suffx, pref;
fr(i,0,n) { cin>>a[i]; lastindx[a[i]] = i;}
int suffxindx;
for(int i = n-1;i>=0;--i){
...
int finans = n-suffxindx; // length of the array elements which are retained
int limit=0;
for(int i=0;i<n;++i){

	if(pref.count(a[i])){break;}
	else{
		pref[a[i]]++;
		limit = max(limit,lastindx[a[i]]);
		if(limit>=suffxindx){
			finans = max(finans, i+1+n-limit-1);

		}
		else {
			finans=max(finans, i+1+n-suffxindx);
		}
