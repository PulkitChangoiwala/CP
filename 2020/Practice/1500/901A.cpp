...
cin>>h;
int n=0;
vector<int> a(h+1); // height from 0 to h
fr(i,0,h+1){
	cin>>a[i]; n+=a[i];
}
vector<int> t1(n);
fr(i,0,h){
	if(a[i]>1 && a[i+1]>1){
		...
		for(int j=0;j<=h;++j){
			...
			parent=node; 
			//one way to produce tree, when a node x has multiple children then it's last  
			//child (say 'l_c') will be parent for 1 length deeper nodes
			// in second tree we can 'l_c-1' as parent for on of those children
			//for these we store node in variable flag 
			if(j==i) {flag=node;} //it's children need to be shuffled
		}
		debug_arr(t1);//print tree t1
		//int k=0;
		fr(j,0,node) //total number of nodes 
			{   if(t1[j]==flag){ cout<<flag-1<<" "; flag = -10; }
				//only one node will have parent flag-1 other will have it's original flag
				//as parent. So we modify flag to unaccessible value (say=-10) so if condition
				//is not satisfied more than once. 
				else	cout<<t1[j]<<" ";
			}
			cout<<endl;
			t1.clear();
