...
store[0] = 0;
fr(i,1,n+1){
	diffr[i] = diffr[i-1] + (a[i+n]==1 ? 1 : -1);
	if(!store.count(diffr[i])){store[diffr[i]] = i;}	
}
diffl[1] = (a[n]==1 ? 1 : -1);
fr1(i,n-1,0){
	diffl[n+1-i] = diffl[n-i] + (a[i]==1 ? 1 : -1);
}
/*debug_arr(a);
debug_arr(diffl);
debug_arr(diffr);
...
int diffin = diffr[n] + diffl[n]; 
if(diffin==0) {cout<<0<<endl; return;}
int ans = 2*n+1;
for(int l=0;l<=n;++l){
	int dr = diffin - diffl[l];
	int r;
	if(store.count(dr)){r = store[dr];}
	else continue;
	ans = min(ans,l+r);
